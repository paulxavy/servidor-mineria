...
def extract_features(files):
    ...
    fn = os.path.abspath('C:\\Users\\Martínez\\Desktop\\m-servidor\\audios'+'/'+str(files.file))
...
def preds(df):
    ...
    modelo = load_model('C:\\Users\\Martínez\\Desktop\\m-servidor\\model21.h5')
...
def separar(b_value, df):
    df = preds(df)
    nombre = b_value
    new_df = df[df['file'].str.startswith(nombre)]
    #print(new_df)
    return new_df

def freq(new_df):
    prediccion = new_df['preds'].mode()[0]
    #print('La predicción más frecuente es:', prediccion)
    return prediccion

def speaker(prediccion):
    names = ['Carlos Orbe', 'Paul Martinez', 'Angie Yepez', 'Edwin Cuascota', 'Elvis Herrera', 'Alejandra Albuja', 'Geovanna Naranjo', 'Yadira Cobos', 'Erick Gordillo', 'Maria Ortega']
    return names[int(prediccion)-1]


def completo(usuario, df):
    separado = separar(usuario, df)
    id_pred = freq(separado)
    name = speaker(id_pred)
    return name
